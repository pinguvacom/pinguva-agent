...
type agentBitrix24Status struct {
	...
	Diagnostics *agentBitrix24Diagnostics   `json:"diagnostics,omitempty"`
}
...
type agentState struct {
	...
	Bitrix24EndpointWatches []string `json:"bitrix24EndpointWatches,omitempty"`
	// Bitrix24LoadDiagnosticsEnabled is supplied only by a compatible server.
	// Keeping it as a pointer preserves the safe old-server default: an agent
	// never starts the extra collector just because an old backend omitted it.
	Bitrix24LoadDiagnosticsEnabled *bool `json:"bitrix24LoadDiagnosticsEnabled,omitempty"`
}

type agentReportResponse struct {
	Bitrix24EndpointWatches        *[]string `json:"bitrix24EndpointWatches"`
	Bitrix24LoadDiagnosticsEnabled *bool     `json:"bitrix24LoadDiagnosticsEnabled"`
}
...
func runAgentLoop(ctx context.Context, cfg agentConfig, logger *log.Logger) error {
	...
	send := func() {
		...
		response, err := postReport(client, collector.serverURL+"/api/agent/report", state.Token, report)
		if err != nil {
			if cfg.EnrollmentToken != "" && isAuthFailure(err) {
				...
				if syncAgentReportSettings(state, response) {
					if err := saveAgentState(cfg.StatePath, state); err != nil {
						logger.Printf("state save failed: %v", err)
					}
				}
...
type bitrix24LocalConfig struct {
	...
	Diagnostics *bitrix24DiagnosticsLocalConfig `json:"diagnostics,omitempty"`
...
func runBitrix24Command(args []string, logger *log.Logger) error {
	...
	case "diagnostics":
		return runBitrix24Diagnostics(args[1:], logger)
	case "bootstrap":
		return runBitrix24Bootstrap(args[1:], logger)
	default:
...
func runBitrix24Configure(args []string, logger *log.Logger) error {
	...
	accessLogFlag := fs.String("access-log", envOr("BITRIX24_ACCESS_LOG", ""), "Comma-separated Nginx or Apache access log paths for local diagnostics")
	diagnosticsFlag := fs.Bool("diagnostics", true, "Enable local Bitrix24 request and MySQL diagnostics")
	...
	config := bitrix24LocalConfig{
		...
		Profiles:    profiles,
		Diagnostics: newBitrix24DiagnosticsLocalConfig(*diagnosticsFlag, *accessLogFlag),
		CreatedAt:   now,
		UpdatedAt:   now,
	}
	if existing, err := loadBitrix24Config(*configPathFlag); err == nil && existing != nil && !existing.CreatedAt.IsZero() {
		config.CreatedAt = existing.CreatedAt
		if strings.TrimSpace(*accessLogFlag) == "" && existing.Diagnostics != nil && len(existing.Diagnostics.AccessLogPaths) > 0 {
			config.Diagnostics.AccessLogPaths = append([]string(nil), existing.Diagnostics.AccessLogPaths...)
		}
	}
...
func loadBitrix24Config(path string) (*bitrix24LocalConfig, error) {
	...
	config.Diagnostics = normalizeBitrix24DiagnosticsConfig(config.Diagnostics)
	return &config, nil
}
...
func collectBitrix24Status(client *http.Client, path string) *agentBitrix24Status {
	...
	status := checkBitrix24Webhook(client, *config)
	if status != nil {
		status.Diagnostics = loadBitrix24Diagnostics(config.Diagnostics)
	}
	return status
}
...
func postReport(client *http.Client, endpoint, token string, report agentReport) (*agentReportResponse, error) {
	body, err := json.Marshal(report)
	if err != nil {
		return nil, err
	}
	...
	var payload agentReportResponse
...
func syncBitrix24EndpointWatches(state *agentState, response *agentReportResponse) bool {
	if state == nil || response == nil || response.Bitrix24EndpointWatches == nil {
		return false
	}
	next := normalizeBitrix24EndpointWatchPaths(*response.Bitrix24EndpointWatches)
	if equalStringSlices(state.Bitrix24EndpointWatches, next) {
		return false
	}
	state.Bitrix24EndpointWatches = next
	return true
}

func syncAgentReportSettings(state *agentState, response *agentReportResponse) bool {
	if state == nil || response == nil {
		return false
	}
	changed := syncBitrix24EndpointWatches(state, response)
	if response.Bitrix24LoadDiagnosticsEnabled == nil {
		return changed
	}
	next := *response.Bitrix24LoadDiagnosticsEnabled
	if state.Bitrix24LoadDiagnosticsEnabled != nil && *state.Bitrix24LoadDiagnosticsEnabled == next {
		return changed
	}
	state.Bitrix24LoadDiagnosticsEnabled = &next
	return true
}
