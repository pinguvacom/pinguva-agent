...
func TestBitrix24MySQLRecognizesGlobalAllPrivileges(t *testing.T) {
	if !bitrix24MySQLHasProcessPrivilege([]string{"GRANT ALL PRIVILEGES ON *.* TO 'root'@'localhost' WITH GRANT OPTION"}) {
		t.Fatal("global ALL PRIVILEGES must include PROCESS")
	}
	if bitrix24MySQLHasProcessPrivilege([]string{"GRANT ALL PRIVILEGES ON `bitrix`.* TO 'monitor'@'localhost'"}) {
		t.Fatal("database-scoped privileges must not imply PROCESS")
	}
}
