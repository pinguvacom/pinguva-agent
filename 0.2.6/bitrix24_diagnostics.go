...
func newBitrix24DiagnosticsLocalConfig(enabled bool, rawPaths string) *bitrix24DiagnosticsLocalConfig {
	...
	if len(paths) == 0 {
		paths = existingBitrix24AccessLogPaths(defaultBitrix24AccessLogPaths())
	}
...
func existingBitrix24AccessLogPaths(paths []string) []string {
	items := make([]string, 0, len(paths))
	for _, path := range compactBitrix24AccessLogPaths(paths) {
...
func runBitrix24Diagnostics(args []string, logger *log.Logger) error {
	...
	diagnostics := collectBitrix24Diagnostics(*config)
...
func collectBitrix24Diagnostics(config bitrix24LocalConfig) *agentBitrix24Diagnostics {
	...
	result := &agentBitrix24Diagnostics{
		...
		AccessLog:     collectBitrix24AccessLogSummary(local.AccessLogPaths, local.WindowMinutes, now),
...
func collectBitrix24AccessLogSummary(paths []string, windowMinutes int, now time.Time) *agentBitrix24AccessLogSummary {
...
func topBitrix24Endpoints(items map[string]bitrix24EndpointCounter, limit int) []agentBitrix24EndpointSummary {
	...
	for path, item := range items {
		values = append(values, entry{path: path, requests: item.requests, errors5xx: item.errors5xx})
	}
	...
	for _, item := range values {
		out = append(out, agentBitrix24EndpointSummary{Path: item.path, Requests: item.requests, Errors5xx: item.errors5xx})
	}
	return out
}
...
func runBitrix24MySQLQuery(binary, query string) ([]string, error) {
	...
	defer cancel()
	command := exec.CommandContext(ctx, binary, "--batch", "--skip-column-names", "--raw", "--connect-timeout=2", "-e", query)
