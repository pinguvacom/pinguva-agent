...
func bitrix24MySQLHasProcessPrivilege(grants []string) bool {
	for _, grant := range grants {
		...
		end := strings.Index(upper, " ON *.*")
		if start < 0 || end <= start+len("GRANT ") {
			continue
		}
		for _, privilege := range strings.FieldsFunc(upper[start+len("GRANT "):end], func(r rune) bool {
			return r == ',' || r == ' ' || r == '\t' || r == '\n'
		}) {
			// PROCESS is a global privilege. ALL PRIVILEGES on *.* includes it.
			if privilege == "PROCESS" || privilege == "ALL" {
				return true
			}
		}
	}
