...
func TestCollectBitrix24AccessLogSummaryRedactsAndAggregates(t *testing.T) {
	...
	summary := collectBitrix24AccessLogSummary([]string{logPath}, 15, now, nil)
...
func TestCollectBitrix24AccessLogSummaryKeepsWatchedRouteWithoutTraffic(t *testing.T) {
	now := time.Now().UTC().Truncate(time.Second)
	logPath := filepath.Join(t.TempDir(), "access.log")
	if err := os.WriteFile(logPath, []byte(""), 0o600); err != nil {
		t.Fatalf("write access log: %v", err)
	}
	summary := collectBitrix24AccessLogSummary([]string{logPath}, 15, now, []string{"/api/contact.availablepoints?token=secret"})
	if summary == nil || len(summary.TopEndpoints) != 1 {
		t.Fatalf("expected watched endpoint, got %+v", summary)
	}
	endpoint := summary.TopEndpoints[0]
	if endpoint.Path != "/api/contact.availablepoints" || !endpoint.Watched || endpoint.Requests != 0 {
		t.Fatalf("unexpected watched endpoint: %+v", endpoint)
	}
}

func TestSyncBitrix24EndpointWatchesKeepsOldServerResponseCompatible(t *testing.T) {
	state := &agentState{Bitrix24EndpointWatches: []string{"/api/old"}}
	if syncBitrix24EndpointWatches(state, &agentReportResponse{}) {
		t.Fatal("missing field from an older server must not alter local watches")
	}
	values := []string{"/api/contact.availablepoints?token=secret", "/api/contact.availablepoints"}
	if !syncBitrix24EndpointWatches(state, &agentReportResponse{Bitrix24EndpointWatches: &values}) {
		t.Fatal("expected valid server watches to update state")
	}
	if len(state.Bitrix24EndpointWatches) != 1 || state.Bitrix24EndpointWatches[0] != "/api/contact.availablepoints" {
		t.Fatalf("unexpected normalized watches: %+v", state.Bitrix24EndpointWatches)
	}
}

func TestSyncAgentReportSettingsKeepsHistoricalCollectionDisabledForOldServer(t *testing.T) {
	state := &agentState{}
	if syncAgentReportSettings(state, &agentReportResponse{}) {
		t.Fatal("an older server response must not enable historical collection")
	}
	if agentBitrix24LoadDiagnosticsEnabled(state) {
		t.Fatal("missing capability must keep historical collection disabled")
	}
	enabled := true
	if !syncAgentReportSettings(state, &agentReportResponse{Bitrix24LoadDiagnosticsEnabled: &enabled}) {
		t.Fatal("expected compatible server capability to be saved")
	}
	if !agentBitrix24LoadDiagnosticsEnabled(state) {
		t.Fatal("expected enabled server capability")
	}
	disabled := false
	if !syncAgentReportSettings(state, &agentReportResponse{Bitrix24LoadDiagnosticsEnabled: &disabled}) {
		t.Fatal("expected server feature disable to be saved")
	}
	if agentBitrix24LoadDiagnosticsEnabled(state) {
		t.Fatal("expected disabled server capability")
	}
}
