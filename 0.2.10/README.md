...
- `/root/.my.cnf` используется только как обычный файл `root:root` с правами
  ...
  файл.
- глобальное `ALL PRIVILEGES ON *.*` корректно распознаётся как включающее
  право `PROCESS`.
...
- `/root/.my.cnf` is used only as a regular `root:root` file with `0600`
  ...
  modifying the file.
- a global `ALL PRIVILEGES ON *.*` grant is correctly recognized as including
