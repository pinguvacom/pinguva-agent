...
func TestBitrix24MySQLLockDiagnosticsAreOptionalAndReadOnly(t *testing.T) {
	diagnostics, _ := collectBitrix24MySQLDiagnosticsWithRunner(newBitrix24MySQLScenarioRunner(t, bitrix24MySQLScenario{
		Performance: []string{"2\t1"},
		Grants:      []string{"GRANT PROCESS ON *.* TO 'monitor'@'localhost'"},
		Foreign:     []string{"0"},
	...
	if diagnostics.Status != "ok" || diagnostics.LockDiagnostics != "ok" || diagnostics.LockWaitCount != 2 || diagnostics.WaitingTransactions != 1 || diagnostics.BlockingTransactions != 1 {
		t.Fatalf("unexpected lock diagnostics: %+v", diagnostics)
	}
}
...
func newBitrix24MySQLScenarioRunner(t *testing.T, scenario bitrix24MySQLScenario) bitrix24MySQLQueryRunner {
	...
	return func(query string) ([]string, error) {
		...
		case strings.Contains(query, "data_lock_waits"):
			if scenario.Locks == nil && scenario.LocksErr == nil {
				return nil, mysqlTestError("compatibility")
			}
			return scenario.Locks, scenario.LocksErr
