# Pinguva Agent Source 0.2.5

Этот каталог содержит исходный код агента Pinguva версии `0.2.5`.
...
Версия `0.2.5` поддерживает безопасную локальную настройку коробочного Bitrix24.
...
- `scope` — проверка выданных REST-прав через `scope`;
...
- `method_discovery` — проверка доступности `user.get` через `method.get` с
  обязательным параметром `name=user.get`; выключен по умолчанию.
